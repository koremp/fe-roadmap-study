...
나머지 연산자를 사용한 표현식 `a % b`는 `a`를 `b`로 나눈 후 그 나머지(*remainder*)를 정수로 반환해줍니다.
...
`x = value`을 호출하면 `value가 x`에 쓰여지고, 이에 더하여 `value`가 반환됩니다.
...
위 예제에서 표현식 `(a = b + 1)`은 a에 값을 할당하고, 그 값인 `3`을 반환합니다. 그리고 반환 값은 이어지는 표현식에 사용됩니다.
