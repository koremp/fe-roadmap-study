...
조건부 연산자는 자바스크립트 연산자 중 유일하게 매개변수가 3개인 연산자입니다. `cond ? resultA : resultB`와 같은 형태로 사용하고, cond가 truthy면 resultA를, 아니면 resultB를 반환합니다.
...
크고 작음을 비교하는 연산자의 피연산자로 *문자열*이 들어오면 *글자 단위*로 크기 비교가 이뤄집니다. 다른 타입의 값이 들어오면 *숫자형*으로 형 변환한 후 비교를 진행합니다.
