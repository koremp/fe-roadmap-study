...
`x`를 `n`번 곱해주는 함수, `pow(x, n)`를 구현하고 있다고 가정해 봅시다.(단, n은 자연수이고, 조건 n≥0을 만족해야 합니다.)
...
스펙이 불완전해서 그런 것이니 더 많은 유스 케이스를 추가해 봅시다.
...
`Chai`는 이 외에도 다양한 `assertion`을 지원합니다.
